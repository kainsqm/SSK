 ...
 * 每天凌晨同步昨天的数据
 * **/
@Component("syncrecordTask")
public class SyncRecordByDayTask {
	...
	private static Logger log = Logger.getLogger(SyncRecordByDayTask.class);
	SimpleDateFormat sdfyes = new SimpleDateFormat("yyyy-MM-dd");

	// 每天凌晨一点执行一次
	@Scheduled(cron = "0 0 1 * * ?")
	public void syncrecordTask() {
		log.info("录音每天凌晨接口同步录音定时任务开始执行......");
		URL baseUrl;
		URL url;
		IWebSDK sdkClient;
		...
		try {
			...
			calendar.add(Calendar.DATE, -1); // 减填负数
			String yesterdayTime = sdfyes.format(calendar.getTime());// 获取昨天的日期
			String startTime=yesterdayTime+" 00:00:00";
			String stopTime="";
			for (int j = 0; j < 24; j++) { //循环23次，同步当天日期   同步录音为一小时
				Calendar calendarnow = Calendar.getInstance();
				calendarnow.setTime(sdf.parse(startTime));
				calendarnow.add(Calendar.HOUR, 1);
				stopTime = sdf.format(calendarnow.getTime());
				...
				if (!sdkReturn.isResult()) { // 判断接口调用 是否出现异常
					...
					for (int i = 0; i < resultArray.size(); i++) {
						...
						try {
							String RecordReference = object
									.getString("RecordReference");
							int isExist = recordInfoService
									.checkExistRecordInfo(RecordReference);
							if (isExist <= 0) { // 判断该录音为新的录音，避免数据出现重合
								RecordInfo recordinfo = new RecordInfo();
								recordinfo.setReservedThree(RecordReference); // 录音流水号
								recordinfo.setCalledid(object
										.getString("CalledID")); // 被叫号码
								recordinfo.setCallerid(object
										.getString("CallerID")); // 主叫号码
								recordinfo.setExtension(object
										.getString("Extension")); // 分机号
								recordinfo.setIsTask("0"); // 是否为任务录音 0表示否
								recordinfo.setRecordLength(Integer
										.parseInt(object
								...
								String StartRecord = time[0] + "-" + time[1]
										+ "-" + time[2]; // 录音开始时间
								String StopRecord = timestop[0] + "-"
										+ timestop[1] + "-" + timestop[2]; // 录音结束时间
								recordinfo.setStartTime(StartRecord); // 开始录音时间
								recordinfo.setStopTime(StopRecord); // 停止录音时间
								recordinfo.setSmallWorkid(object
										.getString("AgentID"));// 小工号
								UserInfo paraminfo = new UserInfo();
								paraminfo.setSmailWorkid(object
										.getString("AgentID"));
								...
								if (null == userinfo) {
									...
									recordinfo.setBigWorkid(userinfo
											.getWorkId()); // 大工号
									recordinfo.setAgentName(userinfo
											.getUserName());// 话务员姓名
									recordinfo.setGroupName(userinfo
											.getGroupName()); // 班组
								}

								recordinfo.setDirectionFlag(object
										.getString("DirectionFlag")); // 呼入呼出
																		// 1代表呼入
								recordinfo.setAgentID(object
										.getString("AgentID"));// 录音接口华为工号
							...
							operateLogService.addLog(Constans.DATASYNC,
									...
									"同步录音接口入库新增异常，异常信息为{0}", new Object[] { e
											.getMessage() }, null);
							log.info("同步录音接口入库新增出现异常:" + e.getMessage()
									+ "录音流水号为"
