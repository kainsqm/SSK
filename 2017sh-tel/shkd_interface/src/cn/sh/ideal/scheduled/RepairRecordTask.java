 ...
 * 宇高录音接口
 * **/
@Component("repairInfoTask")
public class RepairRecordTask {
	...
	private static Logger log = Logger.getLogger(RepairRecordTask.class);
	private String startTime = "2017-01-02 00:00:00";

	// 每天2分钟执行一次
	//@Scheduled(cron = "0 0/2 * * * ?")
	public void repairInfoTask() {
		...
		String stopTime = "";
		String[] maxtime = startTime.split(" ");
		 Date d = new Date();  
	     SimpleDateFormat sdfdate = new SimpleDateFormat("yyyy-MM-dd");  
	     String dateNowStr = sdfdate.format(d); 
		if (maxtime[0].equals(dateNowStr)) {
			log.info("已超过当前时间,时间为" + maxtime[0]);
		} else {
			try {
				...
				url = new URL(baseUrl,
						"http://"+ConfigProperties.getProperty("recordip")+"/VCLogWebSDK/WebSDK?wsdl"); // 创建地址
																				// //
																				// 192.168.4.78
				WebSDK webSDK = new WebSDK(url, new QName(
						"http://tempuri.org/", "WebSDK"));
				...
				try {
					operateLogService.addLog(Constans.DATASYNC,
							Constans.DATASYNC_RECORDINFO,
							"录音接口调用连接出现异常，异常信息为{0}", new Object[] { e
									.getMessage() }, null);
				} catch (Exception e1) {
