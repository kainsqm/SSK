<%@page import="cn.sh.ideal.model.vSysuser"%>
...
<head>
  		...
  		maingrid_gd= $("#record_list").ligerGrid({
  			 columns: [   
					{ display: '操作', isSort: false, minWidth : 80, render: function (rowdata, rowindex, value){
						...
						if(rowdata.examQuery==1 && (rowdata.examineeStatus==2 || rowdata.examineeStatus==5 || rowdata.examineeStatus==6 )){
							return "<a  href='javascript:void(0);' onclick='query_testpaper(\""+rowdata+"\",\""+rowindex+"\")'>查看试卷</a>";
						} else{
					...
					{ display: '开考时间', name: 'toBeginTimeString', align: 'left', minWidth :100,isSort:false},
					{ display: '交卷时间', name: 'toSubmitTimeString', align: 'left', minWidth :100,isSort:false}
	            ], 
	...
	function selectBtn(){
		var url = "${ctx}/controller/examine/getPersonExamsByMain.do";
	...
	function query_testpaper(rowdata, rowindex){
		...
		var uprow = maingrid_gd.getRow(rowindex);
		location.href = "${ctx}/controller/examine/openUpdateViewByMain.do?id="+uprow.pkAutoId+"&queryflag=1";
	}
	...
	<div>
		...
		<form id="queryExamine">
			<table id="search_table" cellpadding="3" cellspacing="3" width="100%" >
				<tr>
					<td class="td_lable">考生工号：</td><td class="td_value"><input type="text" disabled="disabled" value="<%=user.getWorkId()%>"/></td>
