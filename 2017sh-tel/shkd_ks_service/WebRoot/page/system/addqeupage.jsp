	...
	$(function(){
		...
		$.ajax({
			  ...
			  success: function(html){
				  if(html.flag=='yes'){
						$.post("<%=path%>/controller/quesTions/saveQuesTions.do",
								...
								if('200'){
									...
									$.ligerDialog.error('新增失败', '提示', function(yes) {
										window.parent.f_search();
										frameElement.dialog.close();
									});
	...
	<form id="form1" >
		<table>
			...
			<tr>
				...
				<td><input id="fk_codetype_id" name="fkCodetypeId"
					class="td_value" /></td>
				...
				<td class="td_value"><input id="insertTime" name="insertTime"
					...
					value="${questions.insertTime}"  disabled="disabled" />&nbsp;
				...
				<td><input id="fkInsertUserName" name="fkInsertUserName"
					...
					value="${sysuser.userName}" disabled="disabled" /></td>
			</tr>
			...
			<td><label for="pwd1">考题分数：</label></td>
				<td><input id="quesScore" name="quesScore" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"
			...
			<tr><td colspan="5" id="pointout"><span style="color:red">提示</span>：判断题和判断改错题的答案只能输（Y/N）</td></tr>
			<tr>
				<td><br /></td>
			</tr>
