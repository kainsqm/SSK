...
<script type="text/javascript">
	...
	$(function() {
		...
		url:'<%=path%>/controller/examManager/getQuesTionsUpdate.do?exampaperId='+exampaperId,
		width : 'auto',
		height : '100%',
	...
	function f_search() {
		var date1=$('#insert_time_start').val();
   		...
   		var parmItems=[{ name:"quesContent",value:$('#ques_content').val()},
   		               ...
   		               { name:"chooseType",value:$('input:radio[name="chooseType"]:checked').val()},
   		               { name:"exampaperId",value:exampaperId},
   		               { name:"quesStatus",value:$("#ques_status").val()}];
	   		manager.options.parms=parmItems;
	   		manager.options.usePager =true;
	   		manager.options.newPage=1;
	   		manager.options.page='1';
			manager.options.url = '<%=path%>/controller/examManager/getQuesTionsUpdate.do';//必须这么写
    ...
    */
    ...
    function f_getChecked()
    {
    	...
    	$.ajax({
			  url: "<%=path%>/controller/examManager/inputUpdateQuesArea.do?checkedCustomer="+checkedCustomer,
			  type:"post",
			  dataType:"json",
			  async:false,
			  success: function(flag){
				  location.href = "<%=path%>/controller/examManager/queryUpdateQuesAreaUpSeled.do?exampaperId="+exampaperId;
			   }
			});
	...
	<body onkeydown="keydown();">
		<div align="left">
			<table class="search_table" cellpadding="3" cellspacing="3"
				...
				<tr>
					<td class="td_lable" width="60px" style="min-width:60px">题目内容：</td>
					<td class="td_value"><input  type="text" id="ques_content" /></td>
						<td class="td_lable"width="90px">录入人：</td><td class="td_value"><input type="text" id="fkInsertUserName" onclick="openChooseUser()" name="fkInsertUserName" type="text"/>
				...
				<tr>
					...
					<td class="td_lable">选题方式：</td>
					<td class="td_value">
					<c:choose>
						<c:when test="${chooseType eq '1'}">
							<label><input type="radio" name="chooseType" value="0" >当前试卷题目</label>
							<label><input type="radio" name="chooseType" value="1" checked="checked" >题库所有题目</label>
						</c:when>
						<c:otherwise>
							<label><input type="radio" name="chooseType" value="0" checked="checked">当前试卷题目</label>
							<label><input type="radio" name="chooseType" value="1" >题库所有题目</label>
						</c:otherwise>
					</c:choose>
					</td>
				...
				<tr>
					...
					<input type="button" id="query"
						value="查询" />&nbsp;&nbsp;&nbsp;&nbsp;</shiro:hasPermission><input type="button"
						value="重置" onclick="resets()" />&nbsp;&nbsp;&nbsp;&nbsp;
						<shiro:hasPermission name="kssjgl:selected">
						<input type="button"
						value="查看已选试题" id="insertOpen" onclick="f_queryQues()"
						...
						style="width: 109px;" />&nbsp;&nbsp;&nbsp;&nbsp;</shiro:hasPermission><input type="button" onclick="window.location.href='<%=path%>/controller/examManager/toExamManagerPage.do'" value="返回" ></input></td>
				</tr>
			</table>
			</div>
