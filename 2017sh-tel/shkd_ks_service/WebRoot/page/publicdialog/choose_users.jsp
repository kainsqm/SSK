	...
	<script type="text/javascript"> 
		 ...
		 var maingrid_gd ;
			...
			function loadData(resultData){
		  		maingrid_gd= $("#record_list").ligerGrid({
	                ...
	                data:resultData, sortName: 'examId',height:'100%', pageSize:10 ,width:"100%",rownumbers :true,checkbox:true,isAllowHide: false,newPage:1
	             });
		  		$("#pageloading").hide();
			}
		...
		function sumbitUsers(){
			...
			if(selecteds.length==0){
				$.ligerDialog.warn('您未选择任何用户');
				return;
			}
			var listId = window.parent.document.getElementById("listUserIDs").value.split(",");
			var userIds = window.parent.document.getElementById("listUserIDs").value;
			var alluser = window.parent.document.getElementById("listUserName").value;
			if(userIds.length>0) userIds+=","; 
			if(alluser.length>0) alluser+=",";
			for (var i in selecteds) { 
				var flag = true;
				for(var j=0;j<listId.length;j++){
					if(selecteds[i].userId==listId[j]){
						flag = false;
					}
				}
				if(flag){
					userIds += selecteds[i].userId+",";
					alluser += selecteds[i].userName+",";
				}
			};
			window.parent.document.getElementById("listUserIDs").value = userIds.substring(0,userIds.length-1); 
