...
function select_pop(fileurl) {
    if (fileurl.options[fileurl.selectedIndex].value != "") window.open(fileurl.options[fileurl.selectedIndex].value, "_blank", "toolbar=yes,location=yes,menubar=yes,scrollbars=yes,resizable=yes,left=50,height=500,width=700");
    return true;
}
...
ppRoll.prototype.Marquee = function() {
    ...
    if (this.myA.IsPlay == 1) {
        ...
        if (this.myA.direction == "top") {
            if (this.$(this.myA.demo).scrollTop >= this.$(this.myA.demo2).offsetHeight) this.$(this.myA.demo).scrollTop -= this.$(this.myA.demo2).offsetHeight;
            else {
        ...
        if (this.myA.direction == "down") {
            if (this.$(this.myA.demo1).offsetTop - this.$(this.myA.demo).scrollTop >= 0) this.$(this.myA.demo).scrollTop += this.$(this.myA.demo2).offsetHeight;
            else {
                this.$(this.myA.demo).scrollTop--;
            }
        }
        ...
        if (this.myA.direction == "left") {
            if (this.$(this.myA.demo2).offsetWidth - this.$(this.myA.demo).scrollLeft <= 0) this.$(this.myA.demo).scrollLeft -= this.$(this.myA.demo1).offsetWidth;
            else {
        ...
        if (this.myA.direction == "right") {
            if (this.$(this.myA.demo).scrollLeft <= 0) this.$(this.myA.demo).scrollLeft += this.$(this.myA.demo2).offsetWidth;
            else {
                this.$(this.myA.demo).scrollLeft--;
            }
        }
...
function showSysDiv() {
    ...
    maskObjs.style.height = document.body.scrollHeight;
...
/**
*判断startTime是否大于等于endTime,大于返回false,小于返回true
*@author sz
*/
function checkDate(startTime, endTime, split) {
    if (startTime != "" && endTime != "") { //输入不为空时;
        // 对字符串进行处理
        startTime = startTime.split(split);
        endTime = endTime.split(split);
        // 创建 Date 对象
        var myDate1 = new Date(startTime[0], startTime[1], startTime[2]);
        var myDate2 = new Date(endTime[0], endTime[1], endTime[2]);
        // 对日起进行比较
        if (myDate1 <= myDate2) {
            return true;
        } else {
...
*@author lbg 
*/
function queryReset(form) {
    for (i = 0, n = form.elements.length; i < n; i++) {
        var name = form.elements[i].name;
        if (name == 'roleSHFlag') { //过滤角色参数
