...
<script type="text/javascript">
	...
	$(function() {
		...
		//$("#form").ligerForm({});
		contentAddEditor = TT.createEditor("#table [name=desc]");
	});
	...
	function mysubmit(type){
	   ...
	   $.ligerDialog.confirm('您确认修改吗？','提示', function (yes) { 
				    if(yes){	
						$.ajax({
								url:"<%=path%>/controller/notice/updateNotice.do",
								type:"post",
								data:{
										title:title,
										content:content,
	...
	<div align="center" style="height:auto;" id="table">
		<table cellpadding="5" cellpadding="5" width="100%" border="0"
			...
			<tr style="height:40px;">
				...
				<td align="left">
					<input type="hidden" id="pkAutoId" value="${notice.pkAutoId}"/>
			...
			<tr>
				...
				<td><textarea id="content" name="desc" class="ui-textarea" 
						style="resize: auto;width: 250px;height: 100px;"
						validate="{required:true}">${notice.content}</textarea><span style="color: red">*</span>
				</td>
			</tr>
			<tr>
				<td align="right">发布时间：</td>
				<td align="left"><input class="Wdate date_text"  value="${notice.releaseDate}" type="text" id="releaseDate"
					onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'%y-%M-%d'})" /><span style="color: red">*</span>
				</td>
			</tr>
			<tr>
				<td colspan="2" align="center"
					style="padding-top:15px; padding-bottom:15px;"><shiro:hasPermission name='notice:update'><input
					type="button" value="保存" onclick="mysubmit(0)"/></shiro:hasPermission>&nbsp;&nbsp; <shiro:hasPermission name='notice:update'><input type="button"
