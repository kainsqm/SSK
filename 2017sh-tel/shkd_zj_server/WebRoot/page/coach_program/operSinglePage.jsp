...
<script type="text/javascript"> 
	var maingrid_gd="";
		$(function () {
		    var olduserid=window.parent.document.getElementById("userid").value;
            maingrid_gd= $("#workpaper_list").ligerGrid({
				...
				{ display: '', isSort: false, name:'userName', width :20, align:'center', render: function (rowdata, rowindex, value)
	                {
	                    var h = "<input type='radio' name='rd' style='margin-top:8px' onclick='fzname(\""+value+"\",\""+rowdata.userId+"\")'/>";
	                    return h;
	                }
                },
				{ display: '受理员工号', name: 'workId', align: 'left', width :250,isSort:false},
                { display: '受理员姓名', name: 'userName', align: 'left', width :250,isSort:false},
                ],
                url:'<%=path%>/controller/user/getAcceptoruserlist.do',
				sortName: 'workId',
				height:'100%', 
				width:"auto", 
				pageSize:10 ,
        ...
        function search(){
         var parmItems=[{ name:"workId",value:$("#workId").val()},
		    ...
		    maingrid_gd.options.url ='<%=path%>/controller/user/getAcceptoruserlist.do';
			maingrid_gd.loadData(f_getWhere());

	}

	function fzname(username, userId) {
	...
	function chooes() {
		...
		if (username == null || username == "") {
			...
			window.parent.f_xz(username, userid);
			window.parent.document.getElementById("tcc").style.display = "none";
		}
...
<body>
	<div style="">
		...
		<div align="left"
			...
			<input type="hidden" id="name" /> <input type="hidden" id="userid" />
