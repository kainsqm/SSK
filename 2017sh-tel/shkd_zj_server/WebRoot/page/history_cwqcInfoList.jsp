	...
	var dataFromData = [{ directionFlag: 1, text: '合格' }, { directionFlag: 0, text: '不合格'}];
		$(function () {
            ...
            maingrid_gd= $("#history_qc_list").ligerGrid({
				...
				{ display: '操作',name:'recordId', isSort: false, minWidth : 180, render: function (rowdata, rowindex, value)
	                {
	                    var h = "";
						h += "<shiro:hasPermission name='hiscworder:qc'> <a href='javascript:void(0);' onclick='pingfen(\""+rowdata.qcId+"\");'>评分</a> </shiro:hasPermission>";		
						h += "<shiro:hasPermission name='hiscworder:del'> <a href='javascript:delqchis("+rowdata.qcId+")' >删除</a> </shiro:hasPermission>";
	                    return h;
	                }
                },
				...
				{ display: '录音流水号', name: 'reservedThree', align: 'left', width :100,isSort:false},
				{ display: '112工单流水', name: 'workorderId', align: 'left', width :100,isSort:false},
				...
				{ display: '申告大类', name: 'declarationBigType', align: 'left', minWidth :100,isSort:false}
                ], 
                sortName: 'recordId',
                url:'<%=path%>/controller/qchis/getcworderqc.do',
            ...
            $.ajax({
				...
				success:function(data){
					var res=eval("("+data+")");
					$.each(res.bstype,function(index,value){
                        $("#declarationBigTypeid").append("<option value='"+value+"'>"+value+"</option>");
                    })
				},
		...
		function f_search() {
			var parmItems=[{ name:"qcUserWorkid",value:$("#qcUserWorkidid").val()},
			    ...
			    maingrid_gd.options.url = '<%=path%>/controller/qchis/getcworderqc.do';//必须这么写
		...
		<div align="left" style="margin-top:10px; margin-left:5px; padding-left:10px;">
			<table id="search_table" cellpadding="3" cellspacing="3" width="100%" >
				...
				<tr>
					...
					<td class="td_lable">112工单流水：</td><td class="td_value"><input type="text" id="workorderIdid" name="workorderId" onkeyup="input_shuzi(this);strlenLimit(this,20);"/></td>
					<td class="td_lable">申告大类：</td><td class="td_value">
					<select id="declarationBigTypeid" name="declarationBigType" style="width: 135px">
					<option value="">---请选择---</option>
					
					</select>
					
					<!-- <input id="declarationBigTypeid" name="declarationBigType"/> -->
					</td>
					<td class="td_lable">合格程度：</td>
						<td class="td_value" >
							<select id="qualityLevelid" name="qualityLevel" style="width: 135px">
								<option value="">---请选择---</option>
				...
				<td class="td_lable">检查内容：</td>
						<td class="td_value" >
							<select id="checkcontent" name="checkcontent" style="width: 135px">
							<option value="">---请选择---</option>
							</select>
							
						</td>
				<td class="td_lable">组室：</td><td><select name="groupName" id="groupName" style="width: 135px">	
					</select></td>
				...
				<tr>
					<td colspan="8" align="center">
						<shiro:hasPermission name='hiscworder:query'>
