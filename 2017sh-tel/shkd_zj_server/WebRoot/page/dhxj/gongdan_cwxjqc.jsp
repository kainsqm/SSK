...
<head>
	...
	<script type="text/javascript"> 
		...
		$("#businessTypeid").change(function(){
			...
			var isTl9000 = $("input[name='isTl9000']:checked").val();
			$.ajax({
	...
	function loadpassdes(sel){
		...
		if(qcidid!=''&&qualityLevelid=='不合格' ){
			...
			$.ligerDialog.confirm('确认提交评分吗？','提示', function (yes) {
				if(yes){	
					 $.ajax({
						url:'<%=path%>/controller/qc/workordercwQC.do',
						type:'post',
	...
	function closewin(){
	   //判断如果是任务电话小结评分  在点击关闭按钮时，刷新任务录音列表
	   var channelType=$("#channelType").val();
	   if(channelType=="taskcdma"){
	     window.opener.location.reload();
	   }
		 window.close();
	}
...
<body>
	...
	<div align="center" style=" margin:auto; margin-top:10px;">
		...
		<div class="title_div"><label style="margin-left:15px;">c网小结评分</label></div>
		<form id="qcfromid" method="post">
		<input type="hidden" value="${stoptimeq}" id="stoptimeq"/>
		<input type="hidden" value="${stoptimeh}" id="stoptimeh"/>
		<input type="hidden" value="${receiptOpId}" id="agentworkid" name="agentworkid" />
		<!-- 判断进入该页面的渠道  taskcdma 为任务录音评分 -->
		...
		<table cellpadding="3" cellspacing="3" width="1300px" >
			<tr>
				...
				<td class="td_lable" width="80" >受理时间：</td><td class="td_value" width="120"><input class="Wdate date_text" type="text" disabled="disabled" name="startTime" value="${complaintTime}" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'})" /></td>
				...
				<c:if test="${display!='0'}">
				<input  type="text" class="input_text" onblur="noSpecial(this);"  onkeyup="strlenLimit(this,20);noSpecial(this);"  name="deviceId" id="deviceIdid" value="${dirNum}"/><span style="color: red">*</span>
				</c:if>
			...
			<td class="td_lable">备注：</td>
				...
				<tr id="glgdid">
					...
					关联录音信息 <a href="javascript:void(0);" onclick="$('#showGongDanDiv').slideToggle();">关联录音</a><br />
					<div id="showGongDanDiv" style="overflow:hidden; height:350px; margin-top:10px; border:#999999 1px solid;border-style:dashed; padding:10px;" align="center">
						<iframe width="100%" height="370" frameborder="0" scrolling="no" src="<%=path%>/page/112/guanliain_record.jsp"></iframe>
					</div>
				</td>
				</tr>
			<tr>
			...
			<tr>
				...
				<input type="hidden" id="bigWorkid" name="bigWorkid" value="${bigWorkid}"/>
				<input type="hidden" id="smallWorkid" name="smallWorkid" value="${receiptOpId}"/>
				 <input type="hidden" value="${serialCdma}" name="serialCdma" id="serialCdma"/>
				 <input type="hidden" value="${autoid}" name="telsumId" id="telsumId"/>
				</td>
		...
		<shiro:hasPermission name='112xj:senc'>
			<c:if test="${secflag=='0'}">
				<input type="button" value="填写意见" onclick="secqc();" id="secid"/>&nbsp;&nbsp;
			</c:if>
		</shiro:hasPermission>	
		<shiro:hasPermission name='cwxj:check'>
			<c:if test="${subflag!='0'}">
