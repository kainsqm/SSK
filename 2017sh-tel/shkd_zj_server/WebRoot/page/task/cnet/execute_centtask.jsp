...
<script src="<%=path%>/js/util.js" type="text/javascript" language="javascript" ></script>
<style type="text/css">
div.title_div {
	border-radius: 5px;
	text-align: left;
...
<script type="text/javascript"> 
	    var grid=null; 
		$(function () {
			$("#taskStartTime").val(getfirstDay());
			$("#taskEndTime").val(getnowDay());
            grid= $("#task_sys_list").ligerGrid({
                columns: [                
				{ display: '操作', isSort: false, minWidth : 190, render: function (rowdata, rowindex, value)
	                {
	                    var h = "";
	                    h += "<shiro:hasPermission name='ztaskcw:linqu'><a href='javascript:void(0);' onclick='showLingQuPage(\""+rowdata.taskId+"\",\""+rowdata.taskuserId+"\",\""+rowdata.taskName+"\",\""+rowdata.taskStartTime+"\",\""+rowdata.taskEndTime+"\");'>去领小结</a></shiro:hasPermission> ";
					   // if(rowdata.eachQcuserStatus=="1"){
	                   //     h += "";
	                   // }else{
	                        h += "<shiro:hasPermission name='ztaskcw:ylinqu'><a href='javascript:void(0);' onclick='showTaskCdmaList(\""+rowdata.taskId+"\",\""+rowdata.taskName+"\",\""+rowdata.eachQcuserStatus+"\",\""+rowdata.taskuserId+"\");'>质检已领小结</a></shiro:hasPermission> ";
	                        h += "<shiro:hasPermission name='ztaskcw:wcinfo'><a href='javascript:void(0);' onclick='showWcqkForQc(\""+rowdata.taskId+"\",\""+rowdata.taskuserId+"\",\""+rowdata.taskName+"\",\""+rowdata.taskEndTime+"\",\""+rowdata.qcUserName+"\");'>完成情况</a> </shiro:hasPermission>";
	                   // }
	                    return h;
	                }
                },
                ...
                ], url:'<%=path%>/controller/verdict/queryTaskForQc.do',
       ...
       function f_getWhere()
		...
		function showWcqkForQc(taskId,taskQcUserId,taskName,taskEndTime,qcUserName){
			window.location.href="<%=path%>/controller/verdict/showWcqkForQc.do?taskId="+taskId+"&taskName="+encodeURI(encodeURI(taskName))+"&taskQcUserId="+taskQcUserId+"&taskEndTime="+taskEndTime+"&qcUserName="+encodeURI(encodeURI(qcUserName));
		}
		 ...
		 * 打开任务已领录音界面
		 ********************************/
		function showTaskCdmaList(taskId,taskName,eachQcuserStatus,qcUser){
			window.location.href="<%=path%>/page/task/cnet/cnet_yiling_list.jsp?taskId=" + taskId + "&qcUser=" + qcUser+ "&taskName=" + encodeURI(encodeURI(taskName));;
		}
	...
	<div style="">
		...
		<div align="left"
			...
			<table id="search_table" cellpadding="3" cellspacing="3" width="100%">
				<tr>
					<td class="td_lable">任务编号：</td>
					<td class="td_value"><input id="taskId" type="text"
						maxlength="50" onkeyup="input_shuziyinwen(this);" /></td>
					<td class="td_lable">任务名称：</td>
					<td class="td_value"><input id="taskName" type="text"
						maxlength="50" onkeyup="noSpecial(this)" /></td>
					<td class="td_lable">质检员工号：</td>
					<td class="td_value"><input id="qcUserWorkId"
						value="${userworkid}" disabled="disabled" type="text"/></td>
					<td class="td_lable">任务状态：</td>
					<td class="td_value"><select id="taskStatus">
				...
				<tr>
					...
					<shiro:hasPermission
							...
							&nbsp;&nbsp;&nbsp;&nbsp;
						</shiro:hasPermission> <input type="button" value="重置" onclick="chongzhi()" />
