	   ...
	   var maingrid_gd=null;
		$(function () {
          maingrid_gd = $("#task_qc_info").ligerGrid({
				...
				url:'<%=path%>/controller/verdict/getTaskCompleteInfoByDuDao.do?taskId='+$("#taskId").val(),
		 ...
		 *  查看任务完成明细
		 *****************************/
		function showTaskWcqkItems(taskId,taskQcUserId,qcUser,qcUserName){
			window.location.href="<%=path%>/controller/verdict/showTaskWcqkItems.do?taskId="+taskId+"&taskName="+encodeURI(encodeURI($("#task_name").val()))+"&taskQcUserId="+taskQcUserId+"&qcUser="+qcUser+"&qcUserName="+encodeURI(encodeURI(qcUserName));
		}
	   ...
	   if(qcUserItem==""){
		...
		$.ligerDialog.confirm('您确认要释放？', function (yes) { 
		 if(yes){
			...
			jQuery.ajax({
			url:"<%=path%>/controller/verdict/releaseCdmaByUser.do",
			data:parameters,
			type:"post",
			async:false,
			dataType: "json",
