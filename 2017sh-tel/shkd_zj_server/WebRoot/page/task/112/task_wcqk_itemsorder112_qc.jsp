	...
	var maingrid_gd;
		$(function () {
            maingrid_gd= $("#task_qc_info").ligerGrid({
				...
				url:'<%=path%>/controller/task/getwcqkitemsbyworkid.do', 
        ...
        function f_search(){
              var parmItems=[{ name:"taskId",value:$("#taskId").val()}, 
			    ...
			    maingrid_gd.options.url = '<%=path%>/controller/task/getwcqkitemsbyworkid.do',
       ...
       function f_getWhere()
		...
		window.location.href="<%=path%>/page/task/112/task_recordorder112_list.jsp?taskId="+taskUserID+"&cDate="+cDate+"&taskName="+encodeURI(encodeURI(taskName));
		}else{//任务没有领满跳转领取界面
		window.location.href="<%=path%>/page/task/112/task_lingquorder112.jsp?taskId="+taskUserID+"&cDate="+cDate+"&taskName="+encodeURI(encodeURI(taskName));	
		}
