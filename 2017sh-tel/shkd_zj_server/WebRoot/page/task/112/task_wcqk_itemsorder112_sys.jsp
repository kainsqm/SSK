	...
	var maingrid_gd;
		$(function () {
           maingrid_gd = $("#task_qc_info").ligerGrid({
				...
				url:'<%=path%>/controller/task/getwcqkitemsbytaskuserid.do', 
         ...
         function f_search(){
              var parmItems=[{ name:"taskId",value:$("#taskId").val()}, 
			    ...
			    maingrid_gd.options.url = '<%=path%>/controller/task/getwcqkitemsbytaskuserid.do',
	...
	//	var qcUser=document.getElementById("qcUser").value;
		...
		$.ligerDialog.confirm('您确认要释放？', function (yes) { 
		 if(yes){
			...
			jQuery.ajax({
			url:"<%=path%>/controller/task/release112Order.do",
			data:parameters,
			type:"post",
			dataType: "json",
			async:false,
