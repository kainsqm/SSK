	 ...
	 var toendday;
		$(function () {
          ...
          maingrid_gd = $("#task_sys_list").ligerGrid({
				...
				{ display: '操作', isSort: false, minWidth : 180, render: function (rowdata, rowindex, value)
	                {
	                    var h = "";
	                     if(rowdata.isPublish!="0"){
	                        h += "<shiro:hasPermission name='task112ord:info'><a href='javascript:void(0);' onclick='showEidtTask(\"show\",\""+rowdata.taskId+"\");'>查看</a> </shiro:hasPermission>";
	                    }else{
	                        h += "<shiro:hasPermission name='task112ord:upd'> <a href='javascript:void(0);' onclick='showEidtTask(\"update\",\""+rowdata.taskId+"\");'>修改</a> </shiro:hasPermission>";
	                    }
						h += "<shiro:hasPermission name='task112ord:del'> <a href='javascript:void(0);' onclick='deleteTaskInfo(\""+rowdata.taskId+"\",\""+rowdata.taskStatus+"\");'>删除</a> </shiro:hasPermission>";
						 if(rowdata.taskStatus!="0" && rowdata.taskStatus!="1"){
						 	h += "<shiro:hasPermission name='task112ord:wcinfo'><a href='javascript:void(0);' onclick='showTaskWcqk(\""+rowdata.taskId+"\",\""+rowdata.taskName+"\",\""+rowdata.csrTopDCount+"\");'>完成情况</a> </shiro:hasPermission>";
						 }	
	                    return h;
	                }
                },
				...
				{ display: '创建人工号', name: 'qcworkid', align: 'left', minWidth :100,isSort:false}
                 ...
                 url:'<%=path%>/controller/task/gettaskOrder112List.do',
          ...
          function f_search(){
              var parmItems=[{ name:"taskid",value:$("#taskId").val()},
			    ...
			    maingrid_gd.options.url = '<%=path%>/controller/task/gettaskOrder112List.do',
       ...
       function f_getWhere()
		...
		if(editType=="add"){
			   ...
			   window.location.href="<%=path%>/controller/task/to112OrderTaskInfo.do";
			}else{
				window.location.href="<%=path%>/controller/task/update112TaskShow.do?taskId=" +taskID+"&type="+editType ;
		    }	
		}
		/********************************
		 * 打开任务完成情况页面
		...
		window.location.href="<%=path%>/page/task/112/task_wcqk_order112.jsp?taskId="+taskUserID+"&csrTopDCount="+csrTopDCount+"&taskName="+encodeURI(encodeURI(taskName));
		}
		/*********************
		 * 删除任务信息
		 *********************/
		function deleteTaskInfo(taskID,status){
			$.ligerDialog.confirm('确认要删除任务['+taskID+']吗？', function (yes) { 
				if(yes){
					$.ajax({
							url:'<%=path%>/controller/task/del112orderTask.do',
							data:{taskId:taskID,
								  status:status
									},
							type:'post',
	...
	<div style="height: 100%">
		<div align="left" style="margin-top:10px; margin-left:5px; padding-left:10px;overflow:hidden">
		    <ul class="search_ul">
		        ...
		        <li>
		            <label>质检员工号：</label>
		            <input type="text" id="qcUserWorkId" name="qcUserWorkId" />
		        </li>
		        <li>
		            <label>任务状态：</label>
				...
				<li style="width:100%;text-align:center">
				    <shiro:hasPermission name='task112ord:query'>
						<input type="button" value="查询"  onclick="f_search()"/>&nbsp;&nbsp;&nbsp;&nbsp;
					</shiro:hasPermission>	
						<input type="button" value="重置" onclick="reset()" />&nbsp;&nbsp;&nbsp;&nbsp;
					<shiro:hasPermission name='task112ord:add'>
