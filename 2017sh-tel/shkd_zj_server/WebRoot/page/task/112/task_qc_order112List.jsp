	...
	var maingrid_gd;
	 ...
	 var toendday;
		...
		var d = new Date();	
			...
			$("#taskEndTime").val(toendday);
             maingrid_gd= $("#task_sys_list").ligerGrid({
				...
				{ display: '操作', isSort: false, minWidth : 190, render: function (rowdata, rowindex, value)
	                {
	                    var h = "";
	                     if(rowdata.taskStatus=="3" || rowdata.taskStatus=="5"){
	                    	h += "<span style='color:gray'>去领工单&nbsp;&nbsp;</span>";
	                    }else{
	                    h += "<a href='javascript:void(0);' onclick='showLingQuPage(\""+rowdata.taskId+"\",\""+rowdata.taskName+"\",\""+rowdata.taskStartTime+"\",\""+rowdata.taskEndTime+"\");'>去领工单&nbsp;&nbsp;</a> ";
						}
						if(rowdata.taskStatus=="1"){
	                    	h += "<span style='color:gray'>质检已领&nbsp;&nbsp;</span>";
							h += "<span style='color:gray'>完成情况&nbsp;&nbsp;</span>";
	                    }else{
						h += "<a href='javascript:void(0);' onclick='showTaskRecordList(\""+rowdata.taskId+"\",\""+rowdata.taskName+"\");'>质检已领&nbsp;&nbsp;</a> ";
						h += "<a href='javascript:void(0);' onclick='showWcqkForQc(\""+rowdata.taskId+"\",\""+rowdata.taskName+"\",\""+rowdata.csrTopDCount+"\");'>完成情况&nbsp;&nbsp;</a> ";
	                    }
		                    return h;
	                }
                },
				...
				{ display: '创建人工号', name: 'qcworkid', align: 'left', minWidth :100,isSort:false}
                 ...
                 url:'<%=path%>/controller/task/getimptaskOrder112List.do',
                 parms:[/* {name:"taskStartTime",value:$("#taskStartTime").val()},
		               { name:"taskEndTime",value:$("#taskEndTime").val()}, */
          ...
          function f_search(){
              var parmItems=[{ name:"taskid",value:$("#taskId").val()},
			    ...
			    maingrid_gd.options.url = '<%=path%>/controller/task/getimptaskOrder112List.do',
       ...
       function f_getWhere()
		...
		window.location.href="<%=path%>/page/task/112/task_wcqk_itemsorder112_qc.jsp?taskId="+taskUserID+"&csrTopDCount="+csrTopDCount+"&taskName="+encodeURI(encodeURI(taskName));
		}
		/********************************
		 * 打开领取录音界面
		 ********************************/
		...
		var myDate = new Date();
			...
			window.location.href="<%=path%>/page/task/112/task_lingquorder112.jsp?taskId="+taskUserID+"&taskName="+encodeURI(encodeURI(taskName));
		}
		...
		window.location.href="<%=path%>/page/task/112/task_recordorder112_list.jsp?taskId="+taskUserID+"&taskName="+encodeURI(encodeURI(taskName));
		}
		...
		<div align="left" style="margin-top:10px; margin-left:5px; padding-left:10px;">
			<table id="search_table" cellpadding="3" cellspacing="3" width="100%" >
				<tr>
					...
					<input type="text" value="${userworkid}" disabled="disabled"  id="qcUserWorkId" name="qcUserWorkId" />
					</td>
				...
				<tr>
					<td colspan="8" align="center">
						<shiro:hasPermission name='ztask112ord:query'>
