...
<head>
	...
	var maingrid_gd;
		...
		window.location.href="<%=path%>/page/task/cw/task_wcqk_itemsordercw_sys.jsp?taskId="+taskId+"&taskuserId="+taskuserId+"&taskName="+encodeURI(encodeURI(taskName))+"&qcUserName="+encodeURI(encodeURI(qcUserName));
		}
	...
	function releaseRecordItem(){
	   ...
	   if(index==0){   
		...
		//	var parameters="taskId="+taskId+"&qcUserItem="+qcUserItem;
			jQuery.ajax({
			url:"<%=path%>/controller/task/releasecwOrderByUser.do",
			data:{taskId:taskId,
