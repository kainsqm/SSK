	...
	var maingrid_gd;
	 ...
	 var toendday;
		...
		var d = new Date();	
			...
			$("#taskEndTime").val(toendday);
             maingrid_gd= $("#task_sys_list").ligerGrid({
				...
				{ display: '创建人工号', name: 'qcworkid', align: 'left', minWidth :100,isSort:false}
                 ...
                 url:'<%=path%>/controller/task/getimptaskOrdercwList.do',
          ...
          function f_search(){
              var parmItems=[{ name:"taskid",value:$("#taskId").val()},
			    ...
			    maingrid_gd.options.url = '<%=path%>/controller/task/getimptaskOrdercwList.do',
       ...
       function f_getWhere()
		...
		window.location.href="<%=path%>/page/task/cw/task_wcqk_itemsordercw_qc.jsp?taskId="+taskUserID+"&csrTopDCount="+csrTopDCount+"&taskName="+encodeURI(encodeURI(taskName));
		}
		/********************************
		 * 打开领取录音界面
		 ********************************/
		...
		var myDate = new Date();
			...
			window.location.href="<%=path%>/page/task/cw/task_lingquordercw.jsp?taskId="+taskUserID+"&taskName="+encodeURI(encodeURI(taskName));
		}
		...
		window.location.href="<%=path%>/page/task/cw/task_recordordercw_list.jsp?taskId="+taskUserID+"&taskName="+encodeURI(encodeURI(taskName));
		}
		...
		<div align="left" style="margin-top:10px; margin-left:5px; padding-left:10px;">
			<table id="search_table" cellpadding="3" cellspacing="3" width="100%" >
				...
				<tr>
					...
					<shiro:hasPermission name='ztaskcword:query'>
