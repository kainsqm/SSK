 $(function () {
	      ...
	      // type="update";
	       if(type!="update"){
	            ...
	            $('td input[name="back"]').attr("disabled", false);
	            // $("td input").attr("display", "none");
	            $('input[name="IS_PUBLISH"]').attr("disabled", true);
 ...
 /**
		     ...
		     success: function(result){ 
		    	 ...
		    	 if(count>0){
		    		 ...
		    		 $.ligerDialog.confirm('确认提交吗？','提示', function (yes) { 
		    				...
		    				//$("#myform").submit();
		    				 $.ajax({
		    						url:ctx+'/controller/task/updateTask.do',
		    						type:'post',
	...
	// 加载数据
	function init(csrTopWCount,qcDCount,quUser,isPublish){
		document.getElementById("oneCsrWCount1").value=csrTopWCount.split(',')[0];
		document.getElementById("oneCsrWCount2").value=csrTopWCount.split(',')[1];
		document.getElementById("oneCsrWCount3").value=csrTopWCount.split(',')[2];
		document.getElementById("oneCsrWCount4").value=csrTopWCount.split(',')[3];
		document.getElementById("oneCsrWCount5").value=csrTopWCount.split(',')[4];
		document.getElementById("oneCsrWCount6").value=csrTopWCount.split(',')[5];
		document.getElementById("oneQcDCount1").value=qcDCount.split(',')[0];
		document.getElementById("oneQcDCount2").value=qcDCount.split(',')[1];
		document.getElementById("oneQcDCount3").value=qcDCount.split(',')[2];
		document.getElementById("oneQcDCount4").value=qcDCount.split(',')[3];
		document.getElementById("oneQcDCount5").value=qcDCount.split(',')[4];
		document.getElementById("oneQcDCount6").value=qcDCount.split(',')[5];
		var dom= document.getElementsByName("IS_PUBLISH");
		if(isPublish=="0"){
			dom[1].checked="checked";
		}else if(isPublish=="1"){
			dom[0].checked="checked";
		}
		...
		var items=quUser.split(',');
  		for(var i=0;i<fromObjOptions.length;i++){ 
  			for ( var j = 0; j < items.length; j++) {
  				if(fromObjOptions[i].value==items[j]){ 
...
function changeNumber(obj){
	...
	function befSavCheck(){				
		var taskName= document.getElementsByName("TASK_NAME")[0].value;
		var taskStartTime = document.getElementsByName("taskStartTime")[0].value;
		var taskEndTime = document.getElementsByName("taskEndTime")[0].value;
...
// csrCountByOneQc=document.getElementById("csrCountByOneQc").value;//任务期间每个CSR被同一个质检员最高质检次数
	...
	// hmsType:[0]输出时间的时分秒为00：00：00, [1]输出时间的时分秒为23：59：59, [其他]输出时间的时分秒为输入时间的时分秒
		...
		if(t1v != "" && t2v == ""){
		// $.post("<%=path%>/controller/task/timeChaOherCha.do?ymdhms=" + t1v +
		// "&nday=" + nday + "&hmsType=" + hmsType,null,function (data){
		// t2Obj.value =data;
			// });
			$.ajax({
